...
class User(Base):
    """
    Represents a user in the system. Can be authenticated via local password or OAuth providers.
    """
    __tablename__ = "users"
    id: uuid.UUID = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email: str = Column(String, unique=True, index=True, nullable=False)
    username: str = Column(String, unique=True, index=True, nullable=False)
    hashed_password: str = Column(String, nullable=True) # Nullable for OAuth users
    
    # OAuth specific fields
    provider: str = Column(String, nullable=False, default="local") # e.g., 'local', 'google', 'apple'
    social_id: str = Column(String, unique=True, index=True, nullable=True) # User's ID from the provider
